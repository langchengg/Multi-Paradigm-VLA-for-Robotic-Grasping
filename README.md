...
- Real MuJoCo renderer in the sandboxed `python` process: failed with `invalid CoreGraphics connection`
- Real MuJoCo renderer through `/opt/anaconda3/bin/mjpython` outside the sandbox: passed
- Smoke fallback used: `VLA_MUJOCO_FAKE_RENDERER=1`

The fallback still runs MuJoCo physics, Franka control, HDF5 generation, training, checkpointing, and evaluation. It only replaces RGB rendering with black placeholder frames because the sandboxed terminal cannot create a macOS CoreGraphics/OpenGL context.

Real renderer/viewer verification:

```bash
/opt/anaconda3/bin/mjpython -c "from src.envs.synthetic_franka_grasp_env import SyntheticFrankaGraspEnv; env=SyntheticFrankaGraspEnv(image_size=64); obs=env.reset(target_object='red_cube', randomize=False); print(obs['image'].shape, obs['image'].dtype, int(obs['image'].sum())); env.close()"

/opt/anaconda3/bin/mjpython -m src.visualization.live_watch \
  --dataset synthetic \
  --decoder diffusion \
  --checkpoint_dir checkpoints/e2e/synthetic/diffusion \
  --viewer \
  --num_episodes 1 \
  --max_steps 5 \
  --sleep 0 \
...
Observed real-render output:

```text
(64, 64, 3) uint8 1851144
[FrankaGraspEnv] Interactive MuJoCo viewer launched
[live_watch] loaded checkpoints/e2e/synthetic/diffusion/latest.pt
[live_watch] episode=1/1 return=-2.67 success=False
```

Use `mjpython` or a normal macOS GUI terminal for real viewer rendering. Use `VLA_MUJOCO_FAKE_RENDERER=1` only for headless smoke/CI.

Smoke outputs:

```text
data/smoke_synthetic.hdf5
checkpoints/smoke/synthetic/diffusion/latest.pt
results/smoke/synthetic/diffusion/metrics.json
```

I also ran a tiny all-decoder end-to-end check using the same headless fallback:

```bash
VLA_MUJOCO_FAKE_RENDERER=1 python -m src.training.train --dataset synthetic --data_path data/smoke_synthetic.hdf5 --decoder autoregressive --num_epochs 1 --batch_size 2 --horizon 4 --max_batches 2 --no_pretrained_clip --tiny_random_clip --hidden_dim 64 --num_layers 2 --checkpoint_dir checkpoints/e2e --results_dir results/e2e --disable_tensorboard
...
VLA_MUJOCO_FAKE_RENDERER=1 python -m src.training.train --dataset synthetic --data_path data/smoke_synthetic.hdf5 --decoder flow_matching --num_epochs 1 --batch_size 2 --horizon 4 --max_batches 2 --no_pretrained_clip --tiny_random_clip --hidden_dim 64 --num_layers 2 --inference_steps 2 --checkpoint_dir checkpoints/e2e --results_dir results/e2e --disable_tensorboard
VLA_MUJOCO_FAKE_RENDERER=1 python -m src.training.evaluate --dataset synthetic --all_decoders --num_episodes 1 --max_steps 10 --horizon 4 --no_pretrained_clip --tiny_random_clip --hidden_dim 64 --num_layers 2 --diffusion_train_steps 10 --inference_steps 2 --checkpoint_dir checkpoints/e2e --results_dir results/e2e --no_save_video
```

Tiny run metrics are only a wiring check, not a research result:

| Decoder | Train steps | Last loss | Success rate | Avg return | Action MSE | Latency ms | Failure |
|---|---:|---:|---:|---:|---:|---:|---|
| Autoregressive | 2 | 5.5805 | 0.0 | -5.3012 | 0.9138 | 2.3879 | missed_object |
| Diffusion | 2 | 1.0048 | 0.0 | -4.5725 | 0.8736 | 0.9150 | failed_lift |
| Flow-Matching | 2 | 1.8025 | 0.0 | -4.5893 | 0.6462 | 0.8833 | failed_lift |

Detailed files:

```text
checkpoints/e2e/synthetic/*/train_summary.json
results/e2e/synthetic/*/metrics.json
results/e2e/synthetic/comparison_metrics.json
```
